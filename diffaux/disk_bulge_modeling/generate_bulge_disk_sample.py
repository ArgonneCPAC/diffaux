...
from .mc_disk_bulge import mc_disk_bulge, generate_fbulge_params
...
def get_redshifts_from_times(t_table, cosmo_params, zmin=.001, zmax=50, Ngrid=200, zcheck=3):
    ...
    print("Check times calculated from redshifts are within 1e-3 for z< {}: {}".format(
          zcheck, np.all(check[mask])))
    return redshifts
...
def get_bulge_disk_test_sample(ran_key, lgmp_min = 11.0, redshift = 0.05,
    ...
    if centrals:
        diffstar = mc_diffstar_cenpop(*args,
    ...
    diffstar['z_table'] = get_redshifts_from_times(diffstar['t_table'],
                                                   cosmology)
