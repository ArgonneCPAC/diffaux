...
def plot_qs_nocuts(
    ...
    for ax, z in zip_longest(ax_all.flat, zvalues):
        ...
        for q_z, qlabel, color in zip(qs_z, qlabels, colors):
            N, _, _ = ax.hist(q_z, bins=bins, color=color,
                              label=qlabel, histtype="step")
...
def plot_histories(
    ...
    if reverse:
        ...
        print(len(color_array[row_mask]),
              color_array[row_mask][sort_array][::check_step])
    ...
    for ax, q, ylabel in zip_longest(ax_all.flat, qs, ylabels):
        ...
        for n, (h, c, i) in enumerate(
            ...
            if n == 0:
                __ = ax.plot(
                    t_table, h, color=colors[i], label=lgnd_label.format(c))
...
def plot_q_with_cuts(
    ...
    for ax, z in zip_longest(ax_all.flat, zvalues):
        ...
        for n, (cut, cut_label, color) in enumerate(zip(cuts, cut_labels, colors)):
            ...
            ax.hist(q_z[cut_mask], bins=bins, color=color,
                    label=label, alpha=0.4, density=True)
        if yscale == "log":
            ax.set_yscale("log")
        if xscale == "log":
            ax.set_xscale("log")
...
def plot_q1_q2(
    ...
    for ax, z in zip_longest(ax_all.flat, zvalues):
        ...
        if cut_at_z0:  # cut on value at z=0
            cut_mask = (cut_array_z[:, -1] >=
                        cut_lo) & (cut_array_z[:, -1] < cut_hi)
            # now broadcast back to 2-d mask
            cut_mask = np.broadcast_to(
                cut_mask, (np.count_nonzero(zmask), len(cut_mask))).T
        ...
        for q_z, qlabel, color in zip([q1_z[cut_mask], q2_z[cut_mask]], qlabels, colors):
            ax.hist(q_z, bins=bins, color=color,
                    label=qlabel, alpha=0.4, density=True)

        ax.set_yscale(yscale)
        ax.set_xscale(xscale)
        ax.set_xlim(
            max(np.min(bins), min(
                np.min(q1_z[cut_mask]), np.min(q1_z[cut_mask])) * 0.5),
            min(np.max(bins), max(
                np.max(q1_z[cut_mask]), np.max(q1_z[cut_mask]))) * 2.0,
        )
        ax.set_xlabel(xlabel)
        ax.set_ylabel("PDF")
...
def plot_q1_vs_q2(
    ...
    for ax, z in zip_longest(ax_all.flat, zvalues):
        ...
        print(np.min(qx_z[idx]), np.max(qx_z[idx]),
              np.min(qy_z[idx]), np.max(qy_z[idx]))
    ...
    for ax, q, ylabel, error in zip_longest(ax_all.flat, qs, ylabels, errors):
        ...
        for zidx, z, c in zip(zindexes, zs, colors):
            ...
            std, _, _ = binned_statistic(
                xvals, yvals, bins=bins, statistic='std')
            ax.plot(xmeans, ymeans, label=zlabel, color=c)
            if error:
                ax.fill_between(xmeans, ymeans-std, ymeans +
                                std, alpha=0.3, color=c)
...
def plot_eff_sigmoids(tarr, f_tcrit, f_early, f_late, Dt,
    ...
    for ax, early, late in zip(ax_all.flat, f_early, f_late):
        title = "$f_{{early}} = {:.2f}, f_{{late}} = {:.2f}$".format(
            early, late)
        for thalf in f_tcrit:
            for dt in Dt:
                ...
                ax.plot(
                    tarr, yvals, label='$t_c$ = {:.1f}, $dt$ = {:.1f}'.format(thalf, dt))
...
def plot_qs_zprofiles(ax_all, xs, qs, binz, zindexes, zs,
    ...
    for zidx, z, c in zip(zindexes, zs, colors):
        zlabel = "$z = {:.2f}$".format(z)
        xvals = [x[:, zidx] for x in xs] if x_depends_z else [
            x for x in xs]  # x values at z
...
def plot_qs_profiles(xs, fs, labels, lxs, lys, bins,
    ...
    for x, f, label in zip(xs, fs, labels):  # loop over curves for each plot
        plot_q_profile(ax_all, x, f, bins, label,
                       error=True)  # loop over plots
...
def plot_sigmoid_2d(x, x0, y, y0, kpairs, ymin, ymax,
                    ylabel='$\\log_{10}(M^*/M_\\odot)$', xlabel='$\\log_{10}(sSFR/yr^{-1})$',
                    edgecolor='royalblue', ytit=0.95, ysuptit=0.98,
                    alpha=0.5, cmap='coolwarm', contour=False, cmapc='coolwarm_r',
                    plotdir='./', plotsubdir='sigmoids', xname="",
                    title="", pltname="sigmoid_2d_{}.png",
                    ):
    ...
    for ax, (kx, ky) in zip(ax_all.flat, kpairs):
        ...
        label = '$kx = {:.1f}, ky={:.1f}$'.format(kx, ky)
        ax.plot_surface(x, y, z, edgecolor=edgecolor,
                        cmap=cmap,
...
def plot_q_vs_xs_color_scatter(q, xs, color_arrays, qlabel,
                               clabels=("$\\log_{10}(sSFR/yr^{-1})$",
                                        "$\\log_{10}(M^*/M_\\odot)$"),
                               xlabels=("$\\log_{10}(M^*/M_\\odot)$",
                                        "$\\log_{10}(sSFR/yr^{-1})$"),
                               N=2000, wspace=0.5,
                               pltname="{}_vs_sSFR_Mstar_{}.png",
                               xname="", cmaps=["jet_r", "jet"],
                               plotdir="./",
                               plotsubdir="Fbulge",
                               title="",
                               ):
    ...
    for ax, x, c, xlabel, clabel, cmap in zip(ax_all.flat, xs, color_arrays,
                                              xlabels, clabels, cmaps):
...
def plot_comparison_profiles_at_zvalues(xlist, qlist, binz,
                                        labels, zindexes, zs, xlabel, ylabel,
                                        plotdir=plotdir,
                                        title=title, xname=xname, error=True,
