...
def read_re_vs_mstar_data(tmp, data, author, validation_info, sample=""):
    ...
    if not all([cname in data for cname in v["M*_colnames"]]):
    ...
    for n, zlo in enumerate(np.arange(v["z_lo"], v["z_hi"], v["dz"])):
        zhi = zlo + v["dz"]
        # print(n, zlo, zhi)
        zlabel = f"${zlo:.1f} \\leq z < {zhi:.1f}$"
        print(f"Processing {zlabel}")
        if zlabel not in data:
            data[zlabel] = {}
...
def read_re_vs_z_data(tmp, data, author, validation_info):
    # collect M* columns
    v = validation_info[author]
    if "format" in v and "M*_by_row" in v["format"]:
        row = v["M*_rows"]
    ...
    for k, v in data.items():
        if "_row" not in k:
            ...
            assert len(v) == test, f"Mismatch in length of array {k}, {len(v)} != {test}"
...
def read_re_vs_z_mstar_fits(tmp, data, author, validation_info):
    ...
    for n, sample in enumerate(validation_info[author]["samples"]):
        for i, colname in enumerate(v["data_colnames"]):
            ...
            if "flip_sign" in v and colname == v["data_colnames"][v["flip_value_columns"]].format(sample):
                data[colname] = -data[colname]
                print(f"Flipping sign of {colname}")
...
def complete_data_values(data, author, validation_info, samples=""):
    ...
    for sample in samples:
        ...
        if xval not in data and "copy_from_common" in v and v["copy_from_common"]:
            old_x = v["copy_columns"]
            data[xval] = data[old_x]
            print(f"..Copying {old_x} to {xval}")

        for n, (yval, yerr_p, yerr_n) in enumerate(
            zip(validation_info["y-values"], validation_info["y-errors+"], validation_info["y-errors-"])
        ):
            ...
            if yval not in data:
                ...
                if "error_n_columns" in v:
                    err_n_idx = v["error_n_columns"][n]
                    data[yerr_n] = np.zeros(len(data[old_val]))
                    old_err_n = v["data_colnames"][err_n_idx].format(sample)
                mask = data[old_val] != 0
                if "convert_to_log" in v and v["convert_to_log"]:
                    data[yval][mask] = np.log10(data[old_val][mask])
                    print(f"..Converting {old_val} to log")
                    data[yerr_p][mask] = (
                        np.log10(data[old_val][mask] + data[old_err][mask]) - data[yval][mask]
                    )
                    print(f"..Converting {old_err} to log")
                elif "convert_from_log" in v and v["convert_from_log"]:
                    ...
                    print(f"..Converting {old_val} from log to {yval}")
                    if "convert_limits_to_errors" in v and v["convert_limits_to_errors"]:
                        ...
                        data[yerr_p][mask] = np.abs(
                            np.power(10, (data[old_val][mask] + data[old_err][mask])) - data[yval][mask]
                        )
                        # Check for asymmetric errors
                        if "error_n_columns" in v:
                            data[yerr_n][mask] = np.abs(
                                -np.power(10, (data[old_val][mask] - data[old_err_n][mask]))
                                + data[yval][mask]
                            )
                    print(f"..Converting {old_err} from log to {yerr_p}")

                # else Nothing to do
            # Check for negative errors
            if yerr_n not in data:
                data[yerr_n] = data[yerr_p]
                print(f"..Assigning symmetric errors for {yerr_n}")

        # adjustment for pivot point for M* fits
        if "adjust_for_M_p" in v and v["adjust_for_M_p"]:
            Mp_txt = v["M_p_label"]
            yval = validation_info["y-values"][v["M_p_value_column"]].format(sample)
            yerr_p = validation_info["y-errors+"][v["M_p_value_column"]].format(sample)
            yerr_n = validation_info["y-errors-"][v["M_p_value_column"]].format(sample)
            alpha = data[validation_info["y-values"][v["M_p_power_column"]].format(sample)]
            M_p_rescale = np.power(v["M_p_factor"], alpha)
            for q in [yval, yerr_p, yerr_n]:
                data[q + Mp_txt] = data[q] / M_p_rescale
                print(
                    "..Dividing {} by M_p conversion factor {:.1f}**alpha -> {}{}".format(
                        q, v["M_p_factor"], q, Mp_txt
                    )
                )
...
validation_info = {
    ...
    "Re_vs_Mstar": {
        ...
        "titles": [
            "$\\bm{{A}}$: $\\bm{{R_e = A*(M^*/({} \\times 10^{{10}}M_\\odot))^{{\\alpha}}}}$",
        ...
        "kawinwanichakij_2021": {
            ...
            "x_colnames": ["z_med"],
            "x_columns": [0],
            "data_colnames": ["alpha_{}", "dalpha_{}+", "A_{}", "dA_{}+", "sigma_{}", "dsigma_{}+"],
        ...
        "mowla_2019": {
            ...
            "x_colnames": ["z_med"],
            "x_columns": [0],
            "data_colnames": ["logA_{}", "dlogA_{}+", "alpha_{}", "dalpha_{}+"],
            "value_columns": [0],
            "error_columns": [1],
            "convert_from_log": True,
            "adjust_for_M_p": True,
    ...
    "Re_vs_z": {
        ...
        "y-errors+": ["dB_{}+", "dbeta_{}+"],
        "y-errors-": ["dB_{}-", "dbeta_{}-"],
        "xlabels": ["$\\bm{\\log_{10}(M^*/M_\\odot)}$", "$\\bm{\\log_{10}(M^*/M_\\odot)}$"],
        "ylabels": ["$\\bm{B}$ (kpc)", r"$\bm\beta$"],
        "titles": [
            "$\\bm{B}$: $\\bm{R_e = B*(1 + z)^{-\\beta}}$",
    ...
    "Re_vs_Mstar2": {
        ...
        "reader": read_re_vs_z_mstar_fits,
        "plotter": plot_re_vs_z_mstar_fits,
        "y-values": ["rp_{}", "alpha_{}", "beta_{}", "logMp_{}"],
        "y-errors+": ["drp_{}+", "dalpha_{}+", "dbeta_{}+", "dlogMp_{}+"],
        "y-errors-": ["drp_{}-", "dalpha_{}-", "dbeta_{}-", "dlogMp_{}-"],
        ...
        "kawinwanichakij_2021": {
            "samples": ["Quiescent", "Starforming"],
            "colors": ["firebrick", "royalblue"],
            "filename": "kawinwanichakij_Table5a_Re_vs_Mstar_fits.txt",
            "skip_header": 3,
            "x_colnames": ["z_med"],
            "x_columns": [0],
            "data_colnames": ["alpha_{}", "dalpha_{}+", "beta_{}", "dbeta_{}+",
        ...
        "george_2024_5000": {
            "samples": ["Quiescent"],
            "colors": ["crimson"],
            "filename": "George_2024_Table1_5000_Re_vs_Mstar2_fits.txt",
            "skip_header": 4,
            "x_colnames": ["zlo", "zhi"],
            "x_columns": [0, 1],
            "data_colnames": [
...
def read_size_data(
    data, authors, validation_info=validation_info, info_key="Re_vs_z", sample="", size_dir=SIZE_DATA_DIR
):
    # setup data dict
