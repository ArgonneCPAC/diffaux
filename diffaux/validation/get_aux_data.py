...
cosmos_dir = os.environ("COSMOS2020")
restcolor_dir = os.environ("RESTCOLORDATA")
...
def add_rest_color_data(aux_data, restcolor_dir, frame="rest"):
    for k, fn in zip(["BC03", "Brown"], ["{}_39_rest_colors.out", "{}_129_rest_colors.out"]):
        key = "_".join([k, frame])
        aux_data[key] = Table()
...
def read_cosmos2020(
    cosmos_dir,
    cosmos_fn="COSMOS2020_Farmer_processed_hlin.fits",
    ...
    print(f"{len(cosmos2020_gals)} objects")
...
def add_cosmos2020_colors(cosmos2020, filter_names, filter_types, bands, minmax=False):
    for fname, ftyp in zip(filter_names, filter_types):
        for b1, b2 in zip(bands[:-1], bands[1:]):
            ...
            if minmax:
                print(f"{c} min/max = {np.min(cosmos2020[colc]):.3g}/{np.max(cosmos2020[colc]):.3g}")

    return cosmos2020


def select_cosmos2020_data(
    cosmos2020,
    keyname="COSMOS2020 ({})",
    selections_max=(
        ("HSC_{}", 26.0),
    ...
    print(f"Selecting {np.count_nonzero(mask)}/{len(mask)} objects after all cuts\n")
...
def make_selections(cosmos2020, select, mask, bands, minimum=True):
    for b in bands:
        ...
        if key in cosmos2020.colnames:
            ...
            print(f"Selecting {dcount}/{len(dmask)} objects with {key} {msg} {select[1]:.1f}")
