        ...
        for fh in fh_list:
            for k in fh:
                ...
                if len(fh[k][col].shape) > 1:
                    array = fh[k][col] if len(array) == 0 else np.vstack((array, fh[k][col]))
                else:
                    array = np.concatenate((array, fh[k][col]))
        t[col] = array

    if verbose:
        print(f"Table colnames: {t.colnames}, {len(t[t.colnames[0]])} galaxies")
    return t
    ...
    for fh in fh_list:
        ...
        for k in keys:
            ...
            if len(cols) != len(colnames):
                print(f"Length mismatch for step {k}")
...
def add_colors(mags, minmax=True, exclude=("id"), frames=("rest", "obs"), filters=None):
                ...
                filters = filters + list(
                    set(
                        [
                            ...
                            if fr in col and "-" not in col and e not in col
                        ]
                    )
                )
    ...
    for f in filters:
        for fr in frames:
            bands = [col.split("_")[-1] for col in mags.colnames if f in col and fr in col and "-" not in col]
            # check band order
            ...
            for b1, b2 in zip(bands[:-1], bands[1:]):
                ...
                if minmax:
                    ...
                    print(f"...{f}:{c} min/max = {mag_min:.3g}/{mag_max:.3g}")
...
def get_zsnaps(pkldir="cosmology/validsky_new_prod/validsky", pklfile="AlphaQ_z2ts.pkl"):
    home = expanduser("~")
    pklfile = os.path.join(home, pkldir, pklfile)
    with open(pklfile, "rb") as f:
        z2ts = pickle.load(f, encoding="latin1")
    zsnaps = np.asarray(sorted([float(k) for k in z2ts]))
