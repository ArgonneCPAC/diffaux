from ..size_modeling.fit_size_data import get_color_mask
from ..size_modeling.fit_size_data import _sigmoid, median_size_vs_z
    ...
    for ax, z_min, z_max in zip_longest(ax_all.flat, z_bins[0:-1], z_bins[1:]):
        ...
        for sample, pcolor, med_color in zip(samples, pt_colors, med_colors):
            ...
            for author in authors:
                if ztitle not in data[author].keys(
                ) and 'z-values' not in val_info.keys():
                    print(
                        '..Skipping bin {} for {}: no data'.format(
                            ztitle, author))
                    continue
                    ...
                    if np.count_nonzero(mask) > 0:
                        ...
                        if ztitle in data[author].keys():
                            ax.plot(
                                ...
                                label=label)
                            y_upper = v[ycol][mask] + \
                                v[yerrp.format(sample)][mask]
                            y_lower = v[ycol][mask] - \
                                v[yerrn.format(sample)][mask]
...
def plot_fits(fits, data_vectors, samples, val_info, data={}, authors=None,
    ...
    for sample, ax_row, color in zip(samples, ax_all, colors):
        M = data_vectors[val_info['x-values'] + '_{}'.format(sample)]
        for par, ax, dYp, dYn, xlabel, ylabel in zip(parameters, ax_row,
                                                     val_info['y-errors+'], val_info['y-errors-'],
                                                     val_info['xlabels'], val_info['ylabels'],
                                                     ):
...
def plot_Re_median(fit_parameters, samples, lM_lo=9.0, lM_hi=12.0, Nm=12,
                   z_lo=0., z_hi=3., Nz=12, fontsize=14,
                   pltname='Re_median_vs_Mstar_ztrends.png', plotdir=PLOT_DRN,
                   size_func=median_size_vs_z, fit_func=_sigmoid):
    nrow = 1
    ncol = len(samples)
    ...
    for ax, sample in zip(ax_all.flat, samples):
        ...
        for z in zs:
            ...
            ax.plot(logMs, Re, label='z = {:.2f}'.format(z))
